...
export default function workItemTests() {
    ...
    equal('a database address with its password', anonymize('at postgresql://owner:secret@ep-x.neon.tech/db', people), 'at [connection]/db');
    ...
    equal('an address on the private network', anonymize('connect ECONNREFUSED 10.0.3.4:6379', people), 'connect ECONNREFUSED [internal]');
    equal('a phone number', anonymize('Ruf an: +49 170 1234567', people), 'Ruf an: [phone]');
    equal('but not a date or an amount', anonymize('2025-09-23, 1234 g', people), '2025-09-23, 1234 g');
    equal('a key', anonymize('sk-ant-api03-abcdefghijklmnopqrstuv', people), '[key]');
