...
export default async function DraftsPage({ params }: { params: Promise<{ locale: string }> }) {
    ...
    /*
     * The page is for everyone with an account — seeing what is on its way in
     * is not a privilege — but finishing one is a decision about the cookbook,
     * so the button is the admin's. A button that always answers 403 is worse
     * than no button.
     */
    const session = await getSession();
    const mayFinish = Boolean(session.user?.admin);
                    ...
                    {drafts.map((draft) => {
                        ...
                        return (
                            ...
                            >
                                ...
                                {mayFinish && <FinishDraft recipeId={draft.id} />}
                            </li>
                        );
                    })}
                </ul>
