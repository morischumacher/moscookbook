...
3. **HTTP 403 & WAF Resilience**: `fetchPage` in `src/lib/fetchPage.ts` uses modern Chrome browser headers and supports optional fallback via `SCRAPING_PROXY_URL` when target sites block direct server requests.
